...
@mutator
def filter_mutation(random, candidate, args):
    """Return the mutants produced by random generation and filtered on plausibility rules.
    ...
    from maphandler import generate_cand_filter
    mutant = generate_cand_filter(random, candidate, args)
    return mutant
	
@mutator
def repair_mutation(random, candidate, args):
    """Return the mutants produced from candidate with reparation of constraint violation.
    ...
    from maphandler import generate_cand_logical
    mutant = generate_cand_logical(random, candidate, args)
    return mutant
